...
def plot_durations(show_result=False):
    plt.figure(1)
    durations_t = torch.tensor(env.balances, dtype=torch.float)
    if show_result:
        plt.title('Result')
    else:
        plt.clf()
...
def run(random: bool = False) -> None:
    for i_episode in range(num_episodes):
        ...
        for t in count():
            action = select_action(state) if not random else torch.tensor([[env.action_space.sample()]], device=device, dtype=torch.long)
